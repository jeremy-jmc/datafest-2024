...
def generate_opti(df_cajero: pd.DataFrame, verbose=False):
    ...
    umbral = 0.2
    for min_percentage in list(np.arange(0.2, 0.0, -0.01)) + [0]: # [0.2]:
        ...
        for t in range(1, T+1):
            if t == 1:
                ...
                # problem += s[t] == S_0 + lp.lpSum((x[v] - W[v]) for v in range(1, t+1))
...
display(df_final)
print(list(set(id_fails)))
print(len(id_fails))

# -----------------------------------------------------------------------------
# Casos [25, 249] nunca funcan porque no tienen solucion optima
# -----------------------------------------------------------------------------
